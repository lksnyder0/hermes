# Hermes Security Verification Script Design
...
TEST_CONTAINER=$(docker run -d \
    ...
    --tmpfs /tmp:size=50m \
    --label hermes.role=security-test \
    hermes-target-ubuntu:latest \
    sleep 300)
...
```bash
#!/bin/bash
# Hermes Security Verification Script
...
#   --all             Test all hermes containers
...
### 1. Test Scope
**Options**:
- A: Test all hermes containers (finds by label)
