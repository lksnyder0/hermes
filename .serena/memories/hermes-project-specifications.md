...
│   ├── honeypot/
│   │   └── Dockerfile                  # Hermes honeypot (Alpine-based)
│   │
...
container_pool:
  ...
  # images:
  #   - name: "hermes-target-ubuntu:latest"
  #     weight: 50
  #   - name: "hermes-target-alpine:latest"
  #     weight: 30
...
# Run as hermes user (if Docker socket permissions allow)
# USER hermes

ENTRYPOINT ["python3", "-m", "hermes"]
```
...
1. **Hermes Honeypot Container** (SSH Server)
...
**Attacker Flow**:
```
Attacker → Hermes (network enabled) → I/O Proxy → Target Container (network disabled)
```
